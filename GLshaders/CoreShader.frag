...
float ComputeDirectionalShadow(vec4 fragmentPositionLightSpace) {
	...
	float closestDepth         = texture(planeShadowMap, projectiveCoordinates.xy).r;
...
//	float shadow               = currentDepth - bias > closestDepth ? 1.0 : 0.0;
	...
	// PCF
	float shadow = 0.0;
	vec2 texelSize = 1.0 / textureSize(planeShadowMap, 0);
	for (int x = -1; x <= 1; ++x)
	{
		for (int y = -1; y <= 1; ++y)
		{
			float pcfDepth = texture(planeShadowMap, projectiveCoordinates.xy + vec2(x, y) * texelSize).r;
