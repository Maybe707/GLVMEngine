...
namespace GLVM::core {
	void VkConfigInitializer(VulkanResources& resources) {
        auto& descriptorSetsConfig = resources.descriptorSetsConfig;
        auto& descriptorBindingsConfig = resources.descriptorBindingsConfig;
        auto& pipelineConfigs = resources.pipelineConfigs;
        auto& renderPassConfigs = resources.renderPassConfigs;
		...
		*/
		...
		pipelineConfigs[DIRECTIONAL_LIGHT_PIPELINE].bindingDescription               = VulkanVertexLayout::getBindingDescription();
		pipelineConfigs[DIRECTIONAL_LIGHT_PIPELINE].attributeDescriptions            = VulkanVertexLayout::getAttributeDescriptions();
		...
		pipelineConfigs[SPOT_LIGHT_PIPELINE].bindingDescription               = VulkanVertexLayout::getBindingDescription();
		pipelineConfigs[SPOT_LIGHT_PIPELINE].attributeDescriptions            = VulkanVertexLayout::getAttributeDescriptions();
		...
		pipelineConfigs[POINT_LIGHT_PIPELINE].bindingDescription               = VulkanVertexLayout::getBindingDescription();
		pipelineConfigs[POINT_LIGHT_PIPELINE].attributeDescriptions            = VulkanVertexLayout::getAttributeDescriptions();
		...
		pipelineConfigs[HUD_PIPELINE].bindingDescription               = VulkanVertexLayout::getBindingDescription();
		pipelineConfigs[HUD_PIPELINE].attributeDescriptions            = VulkanVertexLayout::getAttributeDescriptions();
		...
		pipelineConfigs[FONT_PIPELINE].bindingDescription               = VulkanVertexLayout::getBindingDescription();
		pipelineConfigs[FONT_PIPELINE].attributeDescriptions            = VulkanVertexLayout::getAttributeDescriptions();
		...
		pipelineConfigs[HUD_SCREEN_PIPELINE].bindingDescription               = VulkanVertexLayout::getBindingDescription();
		pipelineConfigs[HUD_SCREEN_PIPELINE].attributeDescriptions            = VulkanVertexLayout::getAttributeDescriptions();
		...
		pipelineConfigs[UI_PIPELINE].bindingDescription               = VulkanVertexLayout::getBindingDescription();
		pipelineConfigs[UI_PIPELINE].attributeDescriptions            = VulkanVertexLayout::getAttributeDescriptions();
		...
		pipelineConfigs[UI_ICONS_PIPELINE].bindingDescription               = VulkanVertexLayout::getBindingDescription();
		pipelineConfigs[UI_ICONS_PIPELINE].attributeDescriptions            = VulkanVertexLayout::getAttributeDescriptions();
		...
		pipelineConfigs[VIRTUAL_TEXTURES_PIPELINE].bindingDescription               = VulkanVertexLayout::getBindingDescription();
		pipelineConfigs[VIRTUAL_TEXTURES_PIPELINE].attributeDescriptions            = VulkanVertexLayout::getAttributeDescriptions();
		...
		pipelineConfigs[MAIN_RENDER_PIPELINE].bindingDescription               = VulkanVertexLayout::getBindingDescription();
		pipelineConfigs[MAIN_RENDER_PIPELINE].attributeDescriptions            = VulkanVertexLayout::getAttributeDescriptions();
		...
		pipelineConfigs[SDF_PIPELINE].bindingDescription               = VulkanVertexLayout::getBindingDescription();
		pipelineConfigs[SDF_PIPELINE].attributeDescriptions            = VulkanVertexLayout::getAttributeDescriptions();
		...
		descriptorBindingsConfig[21].shaderStageFlag         = VK_SHADER_STAGE_VERTEX_BIT;
		descriptorBindingsConfig[21].binding                 = 0;
		descriptorBindingsConfig[21].shaderDescriptorsNumber = 1;
		descriptorBindingsConfig[21].uboChunkSize            = sizeof(COLLISIONS_DEBUG_UBO);

		pipelineConfigs[COLLISIONS_DEBUG_PIPELINE].vertShader                       = "../VKshaders/debug/collisions/vertDebug.spv";
		pipelineConfigs[COLLISIONS_DEBUG_PIPELINE].fragShader                       = "../VKshaders/debug/collisions/fragDebug.spv";
		pipelineConfigs[COLLISIONS_DEBUG_PIPELINE].bindingDescription               = VulkanVertexLayout::getBindingDescription();
		pipelineConfigs[COLLISIONS_DEBUG_PIPELINE].attributeDescriptions            = VulkanVertexLayout::getAttributeDescriptions();
		...
		renderPassConfigs[COLLISIONS_DEBUG_PIPELINE].attachmentDescriptions[0].samples        = VK_SAMPLE_COUNT_1_BIT;
		renderPassConfigs[COLLISIONS_DEBUG_PIPELINE].attachmentDescriptions[0].loadOp         = VK_ATTACHMENT_LOAD_OP_LOAD;
		renderPassConfigs[COLLISIONS_DEBUG_PIPELINE].attachmentDescriptions[0].storeOp        = VK_ATTACHMENT_STORE_OP_STORE;
		renderPassConfigs[COLLISIONS_DEBUG_PIPELINE].attachmentDescriptions[0].stencilLoadOp  = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
		renderPassConfigs[COLLISIONS_DEBUG_PIPELINE].attachmentDescriptions[0].stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
		renderPassConfigs[COLLISIONS_DEBUG_PIPELINE].attachmentDescriptions[0].initialLayout  = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;
		renderPassConfigs[COLLISIONS_DEBUG_PIPELINE].attachmentDescriptions[0].finalLayout    = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;

		renderPassConfigs[COLLISIONS_DEBUG_PIPELINE].attachmentDescriptions[1].flags          = 0;
		renderPassConfigs[COLLISIONS_DEBUG_PIPELINE].attachmentDescriptions[1].samples        = VK_SAMPLE_COUNT_1_BIT;
		renderPassConfigs[COLLISIONS_DEBUG_PIPELINE].attachmentDescriptions[1].loadOp         = VK_ATTACHMENT_LOAD_OP_CLEAR;
		renderPassConfigs[COLLISIONS_DEBUG_PIPELINE].attachmentDescriptions[1].storeOp        = VK_ATTACHMENT_STORE_OP_DONT_CARE;
		renderPassConfigs[COLLISIONS_DEBUG_PIPELINE].attachmentDescriptions[1].stencilLoadOp  = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
		renderPassConfigs[COLLISIONS_DEBUG_PIPELINE].attachmentDescriptions[1].stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
		renderPassConfigs[COLLISIONS_DEBUG_PIPELINE].attachmentDescriptions[1].initialLayout  = VK_IMAGE_LAYOUT_UNDEFINED;
		renderPassConfigs[COLLISIONS_DEBUG_PIPELINE].attachmentDescriptions[1].finalLayout    = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;

		renderPassConfigs[COLLISIONS_DEBUG_PIPELINE].actualAttachmentReferenceNumber          = 2;
		renderPassConfigs[COLLISIONS_DEBUG_PIPELINE].attachmentReferences[0].attachment       = 0;
		renderPassConfigs[COLLISIONS_DEBUG_PIPELINE].attachmentReferences[0].layout           = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
		...
		pipelineConfigs[SPACIAL_GRID_DEBUG_PIPELINE].bindingDescription               = VulkanVertexLayout::getBindingDescription();
		pipelineConfigs[SPACIAL_GRID_DEBUG_PIPELINE].attributeDescriptions            = VulkanVertexLayout::getAttributeDescriptions();
		...
		*/
        ...
        // Screen passes share color/depth attachments. Synchronize both the
        // incoming loads/layout transitions and the outgoing stores/transitions.
        for (unsigned int i = HUD_PIPELINE; i < PIPELINES_NUMBER; ++i) {
            auto& pass = renderPassConfigs[i];
            const VkPipelineStageFlags stages = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT |
                VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT | VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT;
            const VkAccessFlags writes = VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT |
                VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT;
            const VkAccessFlags accesses = writes | VK_ACCESS_COLOR_ATTACHMENT_READ_BIT |
                VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT;
            pass.actualSubpassDependencyNumber = 2;
            pass.subpassDependencies[0] = {VK_SUBPASS_EXTERNAL, 0, stages, stages, writes, accesses, VK_DEPENDENCY_BY_REGION_BIT};
            pass.subpassDependencies[1] = {0, VK_SUBPASS_EXTERNAL, stages, stages, writes, accesses, VK_DEPENDENCY_BY_REGION_BIT};
        }
	}
